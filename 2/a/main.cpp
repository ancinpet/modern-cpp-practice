...
struct hash
{
   size_t operator()(const T& tuple)
   {
      return std::hash<std::tuple_element_t<I, T>>{}(std::get<I>(tuple)) ^ hash<T, N, I + 1>{}(tuple);
   }
};
...
struct hash<T, N, N>
{
   size_t operator()(const T& tuple)
   {
      return std::hash<std::tuple_element_t<N, T>>{}(std::get<N>(tuple));
   }
};
